...
def try_place_card(face_up_pile, inc_stacks, dec_stacks):
    ...
    # ROUND END CONDITION: if we can't place the card, we lose immediately
    return False
...
def compare_with_expected(collected_output, expected_file_path):

    with open("my_output.txt", "w", encoding="utf-8") as f:
        for line in collected_output:
            f.write(line + "\n")

    with open("my_output.txt", "r", encoding="utf-8") as f1, open(expected_file_path, "r", encoding="utf-8") as f2:
        current_lines = f1.readlines()
        expected_lines = f2.readlines()

    diff = difflib.ndiff(current_lines, expected_lines)

    differences = [line for line in diff if line.startswith('- ') or line.startswith('+ ')]

    if not differences:
        print(f"✅ Success! Your output matches {expected_file_path} perfectly.")
    else:
        print(f"❌ Differences found in {expected_file_path}:")
        # Print the first 10 differences to avoid overwhelming the console
...
if __name__ == "__main__":
    try:
        ...
        round_output.pop()
        inputs = ["Q", "+1", "     1", "1---3", "1--780", "1 -- 3", "-4", "q"]
        start_interactive_session(round_output)
        # compare_with_expected(round_output, "expected_solitaire_2_3.txt")
